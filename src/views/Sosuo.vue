  ...
  <div class="s">
    ...
    <div class="search">
      <div class="search-con">
        <div class="ic"></div>
        <input type="text" placeholder="元尊" class="input" @input="suo" v-model="search" />
        <div class="ca"></div>
      </div>
      <div class="search-btn" @click="gorun(search)">确定</div>
    </div>
    <div class="tag">
      <div @click="huan">换一批</div>
      <ul>
        <li v-for="(item, index) in label" :key="index">{{item.word}}</li>
      </ul>
    </div>
    <div class="letters">
      <ul>
        <li v-for="(le, index) in letter" :key="index">{{le}}</li>
      </ul>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    suo() {
      ...
      getSupplement({ query: this.search }).then(
        res => {
