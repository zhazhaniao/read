<template>
 <div class="ranklist">
    <div class="navgation">
        ...
        <div class="select">
            <ul class="select-bar">
                <li v-for="(item,index) in largeTypes" :key="index" :class="{'active':index===largeTypeIndex}"
    ...
    methods: {
      ...
      this.ovo=true;
         ...
         setTimeout(()=>{
                ...
                getRank(this.linkid).then((res) => {
                    if (res.data.ok) {                
                    this.looklist.push(...res.data.ranking.books.slice(this.jiazai,this.jiazai+20));
                    console.log(this.looklist,'QAQ');
                    this.ovo=false
                    }
                }, 
